...
namespace NeuralNet
{
    public class Neuron
    {
        ...
        public Neuron(int numberOfOutputs, int myIndex, List<double> w)
        {
            _myIndex = myIndex;
            ...
            {
                var weight = w[c];//Helpers.GetRandomWeight(0, 0.5);
                Console.WriteLine($"Creating connection for output number {c} with weight {weight}");
        ...
        public void UpdateInputWeights(Layer previousLayer)
        {
            ...
            foreach (var neuron in previousLayer.Neurons)
            {
                ...
                //Console.WriteLine($"New delta: {newDeltaWeight}");
